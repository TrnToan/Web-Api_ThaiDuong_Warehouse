...
public class AddEntryToFinishedProductReceiptCommandHandler : IRequestHandler<AddEntryToFinishedProductReceiptCommand, bool>
{
    ...
    public async Task<bool> Handle(AddEntryToFinishedProductReceiptCommand request, CancellationToken cancellationToken)
    {
        ...
        foreach (var addedEntry in request.Entries)
        {
            ...
            // ReceivedQuantity = ChangedQuantity = Quantity của entry mới thêm vào trong trường hợp bổ sung entry
            goodsReceipt.AddLogEntry(item, addedEntry.Quantity, addedEntry.Quantity, goodsReceipt.Timestamp);
            goodsReceipt.AddFinishedProductInventory(item, addedEntry.PurchaseOrderNumber, addedEntry.Quantity, goodsReceipt.Timestamp);
        }
