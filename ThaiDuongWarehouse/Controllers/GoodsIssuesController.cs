...
public class GoodsIssuesController : ControllerBase
{
    ...
    [HttpDelete]
    [Route("{goodsIssueId}/goodsIssueEntry")]
    public async Task<IActionResult> RemoveEntryAsync(string goodsIssueId, string itemId, string unit)
    {
        RemoveGoodsIssueEntryCommand command = new RemoveGoodsIssueEntryCommand(goodsIssueId, itemId, unit);
        try
        {
            var result = await _mediator.Send(command);
    ...
    [HttpDelete]
    [Route("{goodsIssueId}")]
    public async Task<IActionResult> RemoveAsync(string goodsIssueId)
    {
        RemoveGoodsIssueCommand command = new RemoveGoodsIssueCommand(goodsIssueId);
        try
        {
            var result = await _mediator.Send(command);
